...
void HcalLayerDepthMap::initialize(int subdet, int ietaMax, int zside, 
...
int HcalLayerDepthMap::getDepth16(int subdet, int iphi, int zside) const {
  ...
  if (subdet == subdet_) {
    if (subdet == 1) {  // HB
      depth = getDepth(subdet,ietaMax_,iphi,zside,maxLayers_);
    } else {            //HE
      depth = getDepth(subdet,ietaMin_,iphi,zside,0);
    }
  }
#ifdef EDM_ML_DEBUG
...
bool HcalLayerDepthMap::isValid(int subdet, int iphi, int zside) const {
  bool flag(false);
  if (subdet == subdet_ && zside == zside_)
    flag = (std::find(iphi_.begin(),iphi_.end(),iphi) != iphi_.end());
  return flag;
}

int HcalLayerDepthMap::validDet(int& zside, std::vector<int>& phi) const {
