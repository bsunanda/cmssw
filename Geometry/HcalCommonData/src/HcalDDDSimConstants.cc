...
int HcalDDDSimConstants::getDepthEta16M(int det) const {
  ...
  int zside(-1);
  std::vector<int> phis;
  int detsp = ldmap_.validDet(zside, phis);
  if (detsp == det) {
    int depthsp = ldmap_.getDepth16(det,phis[0],zside);
...
int HcalDDDSimConstants::getDepthEta29M(int i) const {
  ...
  if (i == 0) {
    ...
    int detsp = ldmap_.validDet(zside, phis);
    if (detsp == 2) {
      int depthsp = ldmap_.getMaxDepthLastHE(2,phis[0],zside);
  ...
  if ((int)(hpar->noff.size()) > (noffsize+4)) {
	...
	if ((int)(hpar->noff.size()) >= (noffk+7+nphi+3*ndeps)) {
	  ...
	  std::cout << "Initialize HcalLayerDepthMap for Detector " << dtype
		    << " etaMax " << hpar->etaMax[dtype] << " zside " << zside
		    << " with " << nphi << " sectors";
	  ...
	  std::cout << "Maximum depth for last HE Eta tower " << depthEta29[0]
		    << ":" << ndp29 << " L0 Wt " << hpar->Layer0Wt[dtype-1] 
		    << ":" << wt << std::endl;
#endif
	  ldmap_.initialize(dtype,hpar->etaMax[dtype-1],zside,ndp29,wt,ifi,iet,
			    ily,idp);
	  depthMaxSp_ = std::pair<int,int>(dtype,ldmap_.getDepthMax(dtype,ifi[0],zside));
	}
      }
    }
  }
